...
// 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
...
async function getShopifyOrder(orderId) {
  try {
    // pecaso9063@tospage.com && Test@123
...
async function findOrCreatePerson(email) {
  try {
    ...
    const response = await axios.get(
      `https://api.pipedrive.com/v1/persons/find?term=${email}&api_token=${PIPEDRIVE_API_KEY}`
    );
    ...
    console.error(
      "Error finding/creating person in Pipedrive:",
      error.response.data
    );
...
async function findOrCreateProduct(sku) {
  try {
    ...
    const response = await axios.get(
      `https://api.pipedrive.com/v1/products/find?term=${sku}&api_token=${PIPEDRIVE_API_KEY}`
    );
    if (response.data.data.length > 0) {
      // Product found
      return response.data.data[0];
      ...
      const createResponse = await axios.post(
        `https://api.pipedrive.com/v1/products?api_token=${PIPEDRIVE_API_KEY}`,
        newProduct
      );
      return createResponse.data.data;
    }
    ...
    console.error(
      "Error finding/creating product in Pipedrive:",
      error.response.data
    );
